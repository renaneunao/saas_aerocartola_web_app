...
class CalculoLateral {
    constructor(data) {
        if (window.BLOQUEAR_CALCULO_LATERAL === true) {
            console.warn(`[CALCULO LATERAL] ⚠️ AVISO: O cálculo em tempo real está BLOQUEADO porque um ranking salvo foi encontrado.`);
        }
    ...
    calcularPontuacao(atleta, totalEscalacoes) {
        ...
        if (base_bruta_non_neg > 0) {
            ...
            const scouts = [
                ...
                { val: media_ff, peso: this.pesos.FATOR_FF || 1.0 },
                { val: media_fs, peso: this.pesos.FATOR_FS || 1.0 },
                { val: media_fd, peso: this.pesos.FATOR_FD || 1.0 },
                { val: media_g, peso: this.pesos.FATOR_G || 2.0 },
                { val: media_a, peso: this.pesos.FATOR_A || 2.0 }
            ];
            scouts.forEach(s => {
                if (s.val > 0) {
